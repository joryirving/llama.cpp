...
void llama_model_bailingmoe3::load_arch_hparams(llama_model_loader & ml) {
    ...
    // BailingMoE3 GGUFs exist in two compatible forms: older conversions store
    // clamp values for the decoder layers only, while current conversions append
    // the MTP layer and therefore use n_layer_all entries. Accept both layouts so
    // existing production quants remain loadable alongside newly converted files.
    auto load_clamp_values = [&](enum llm_kv key, auto & values) {
        values.fill(0.0f);

        uint32_t n_values = 0;
        if (ml.get_arr_n(key, n_values, false)) {
            if (n_values != hparams.n_layer() && n_values != hparams.n_layer_all) {
                throw std::runtime_error(
                    "BailingMoE3 clamp array has unsupported length " + std::to_string(n_values) +
                    "; expected " + std::to_string(hparams.n_layer()) + " or " +
                    std::to_string(hparams.n_layer_all));
            }
            ml.get_key_or_arr(key, values, n_values, false);
            return;
        }

        ml.get_key_or_arr(key, values, hparams.n_layer_all, false);
    };

    load_clamp_values(LLM_KV_SWIGLU_CLAMP_EXP,   hparams.swiglu_clamp_exp);
    load_clamp_values(LLM_KV_SWIGLU_CLAMP_SHEXP, hparams.swiglu_clamp_shexp);
